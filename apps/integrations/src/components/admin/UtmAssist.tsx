...
// The fields of the landing page's event shape this tool renders (the events
// proxy passes the landing /api/events payload through unchanged).
...
/** Builds an absolute, UTM-tagged URL for the given landing-site path. */
function buildUrl(origin: string, path: string, utm: UtmFields): string {
  return applyUtm(new URL(path, origin), utm);
}
...
export function UtmAssist({ origin, blogPosts }: UtmAssistProps) {
  // Auth is handled server-side by AdminLayout; this flips when an /api/admin
  // call comes back 401/403 mid-session (expired cookie).
  ...
  // Load the shared campaign list on mount.
  ...
  if (sessionExpired) {
    return (
      <div className="max-w-md mx-auto text-center py-16 px-4">
        ...
        <a
          href="/api/auth/login?returnUrl=%2Fadmin%2Futm-assist"
