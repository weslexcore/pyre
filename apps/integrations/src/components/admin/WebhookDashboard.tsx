// Webhook execution log, ported from the landing-page admin. Auth is handled
// server-side by AdminLayout; a 401/403 from the API mid-session renders a
// re-login prompt instead of a client-side gate.
...
// This dashboard runs on the integrations service, so event links point at the
// public site absolutely.
const eventHref = (siteOrigin: string, id: string) =>
  `${siteOrigin}/events?event=${encodeURIComponent(id)}`;
...
function ExpandedRow({ record, siteOrigin }: { record: WebhookExecution; siteOrigin: string }) {
      ...
      <div className="grid grid-cols-2 gap-4 max-w-lg">
        ...
        {session && (
          <div>
            ...
            <p>
              <a
                href={eventHref(siteOrigin, session.sessionId)}
                target="_blank"
...
export function WebhookDashboard({
  siteOrigin = 'https://pyresauna.com',
}: {
  siteOrigin?: string;
}) {
  ...
  if (error === 'session_expired') {
    return (
      <div className="max-w-md mx-auto text-center py-16 px-4">
        ...
        <a
          href="/api/auth/login?returnUrl=%2Fadmin%2Fwebhooks"
                    ...
                    <tr key={record.id} className="group">
                      <td colSpan={5} className="p-0">
                        ...
                        {expandedId === record.id && (
                          <ExpandedRow record={record} siteOrigin={siteOrigin} />
                        )}
                      </td>
                    </tr>
                  );
