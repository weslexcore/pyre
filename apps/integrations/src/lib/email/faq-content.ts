...
//
// Session-type keys match the output of `deriveSessionType` in
// `src/lib/momence-events.ts` (e.g. 'guided', 'social', 'special event',
// 'open hours', 'fitness', …). Types not listed in FAQS_BY_TYPE — including
// 'open hours' — fall back to DEFAULT_FAQS, so every confirmation always shows
// a sensible FAQ block.

import type { FaqItem } from '@/emails/types';

const WHAT_TO_BRING: FaqItem = {
