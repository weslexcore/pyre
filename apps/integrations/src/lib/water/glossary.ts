...
export const GLOSSARY: Record<Parameter, GlossaryEntry> = {
  ...
  salt: {
    ...
    what: `The dissolved salt content of the water — and the raw material the tub's salt cell turns into chlorine. The tubs are filled with ${PRODUCTS.salt}, roughly 920 g into a fresh ${TUB_GALLONS} gallon fill, and read with the TF-Pro Salt kit.`,
    why: 'Salt is doing two jobs at once. The salt cell passes a current through the water and splits the dissolved salt, generating chlorine right there in the plumbing — so this reading is what decides whether the tub can make its own sanitizer at all. Let salt fall below the range and the cell underproduces or stops, and free chlorine drifts down with it no matter how good the last FC reading looked. Salt is also most of what makes the water feel the way guests expect: softer on skin, easier on eyes and hair than plain tap water. Too much and it turns on the tub instead — salt at high concentration attacks jets, fittings, the chiller loop and the cell plates themselves.',
    chemistry:
      'The cell does not consume salt. The chlorine it generates reverts to salt once it has done its sanitizing, so the same salt cycles round and round; what actually leaves is splash-out, what guests carry out on skin and towels, and draining. That means salt drifts down slowly and never climbs back on its own. A cell only keeps up with routine demand, and only in balanced water — high pH scales the plates and cuts output — so low salt usually shows up first as chlorine that will not hold, and shock days still call for dosed sanitizer rather than more salt.',
    correcting: `Below target: ${PRODUCTS.salt} dosed toward the middle of the range, dissolved in a bucket of water first and poured in front of the filter basket. Give it a full circulation cycle before retesting, and expect chlorine to recover over the following hours as the cell catches up rather than immediately. Above the hard limit: drain some water, refill to dilute, then retest.`,
  },
};
