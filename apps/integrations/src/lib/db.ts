...
// Something a guest left behind, logged from /admin/lost-found. `logged_by`
// and every other identity column are set by the route from the session,
// never from a request body. Unclaimed items are flagged for donation after
// 30 days; the flag is automatic, the donation never is.
export interface LostFoundItemRow {
  id: string;
  /** Human-quotable case number, generated by the database: LF-2026-0001. */
  reference: string;
  status:
    | 'unclaimed'
    | 'claim_pending'
...
// A photo of a lost-and-found item. Objects live in the private
// lost-found-media bucket; storage_path is only ever exchanged for a
// short-lived signed URL by /api/admin/lost-found-media.
export interface LostFoundAttachmentRow {
  id: string;
  item_id: string;
  storage_path: string;
  file_name: string;
...
// One person we asked about one item, and what they said. The claim link in
// the email carries this row's id (signed), so a click identifies the
// claimant without asking them to type anything.
export interface LostFoundNoticeRow {
  id: string;
  item_id: string;
  email: string;
  name: string | null;
  ...
  response: 'claimed' | 'not_mine' | null;
}

// One entry in an item's append-only audit trail.
export interface LostFoundEventRow {
  id: string;
  item_id: string;
  action:
    | 'created'
