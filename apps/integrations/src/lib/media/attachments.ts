// Attachment rules shared by every tool that takes a photo off a phone —
// incident reports, shift notes, and lost-and-found items. The upload routes
// and the form islands both import this, so they agree on what is acceptable
// before a byte moves.
//
// This module is deliberately tool-agnostic: the only per-tool decision is
// the object key prefix, which each caller builds with `storagePathFor`.
//
...
/** Mirrors allowed_mime_types on the media buckets. */
...
/** Same ceiling as every bucket's file_size_limit — 50 MB. */
...
/** Object key for an upload: `<prefix>/<owner id>/<random>.<ext>`. */
export function storagePathFor(
  ...
  return `${prefix}/${ownerId}/${objectName(fileName, mime)}`;
}
...
/**
 * Shrink a phone photo before upload so filing over the site's wifi doesn't
 * stall on a 12 MP original. Returns the file untouched when it is already
 * small, isn't a re-encodable image, or the browser can't decode it (HEIC
 * outside Safari) — the upload still works, it's just bigger.
 */
