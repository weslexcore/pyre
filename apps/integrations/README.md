...
| `/admin/schedule-lint` | Operations | The checks run against the Momence schedule: switch built-in rules off or tune them, add opening-hours / required-tag / expected-capacity / duration-variants rules, preview what they catch, mark a finding resolved so it stops being emailed, run the email by hand |
