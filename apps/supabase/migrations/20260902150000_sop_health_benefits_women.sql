-- Second version of the health benefits SOP. Two changes, recorded as one
-- versioned save:
--
--   * Restructured as a quick reference first (bottom line, evidence tag,
--     link to the detail, links to the studies) with the science in depth
--     below it, so staff can scan for an answer and come back for the
--     research. Headings are prefixed (Sauna: / Cold:) so every in-document
--     anchor is unique.
--   * Adds 'Cold plunging for women', which staff field a lot of questions
--     about: sex differences in cold physiology, the menstrual cycle,
--     perimenopause, pregnancy, and the research gap. Same standard as the
--     rest of the document: peer-reviewed sources only, every claim linked,
--     honest about how thin the evidence is.
--
-- The whole body is replaced rather than spliced so the result is exactly
-- what was reviewed. Guarded on the quick-reference heading so a re-run, or
-- a document an admin has already moved to this structure, is a no-op.

with updated as (
  update public.sops
  set
    content_md = $health2$*A quick reference for answering guest questions, backed by the research it comes from. Scan the first half for the bottom line; follow the links when you want the detail or the study itself.*
...
- **How long, how cold.** Trials mostly use 10 to 15°C water for one to ten minutes. First-timers should think in seconds, not minutes, and build up over visits. [[15]](https://pubmed.ncbi.nlm.nih.gov/39879231/) [[32]](https://pubmed.ncbi.nlm.nih.gov/31702722/)
...
- **No need to time plunges to the menstrual cycle.** Core temperature runs 0.3 to 0.7°C higher in the second half of the cycle, but every direct test of cold immersion across the cycle found no difference in response. Moderate. [Detail](#women-the-menstrual-cycle) · [[39]](https://pubmed.ncbi.nlm.nih.gov/33123618/) [[40]](https://pubmed.ncbi.nlm.nih.gov/10902935/)
...
- **Core temperature shifts across the cycle.** It runs 0.3 to 0.7°C higher in the luteal phase (after ovulation), when progesterone is high, and oestrogen and progesterone move the thresholds at which the body starts constricting or dilating skin blood vessels [[39]](https://pubmed.ncbi.nlm.nih.gov/33123618/) [[38]](https://pubmed.ncbi.nlm.nih.gov/35655670/).
- **But cold immersion responses do not change.** Women tested in 20°C water in both the follicular and luteal phases responded the same in both, and the same as men. The authors concluded that "when faced with a cold challenge, women respond similarly to men in both phases of their menstrual cycle" [[40]](https://pubmed.ncbi.nlm.nih.gov/10902935/).
...
$health2$,
  ...
  where slug = 'health-benefits-sauna-cold-plunge'
    and content_md not like '%## Quick reference%'
...
select id, current_version, title, content_md, 'seed',
  'Restructured as a quick reference with the science in depth; added cold plunging for women'
from updated;
