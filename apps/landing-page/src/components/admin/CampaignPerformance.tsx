...
export function CampaignPerformance() {
  ...
  }, [isAuthenticated, days, fetchReport]);
  ...
  if (!isAuthenticated || !user) {
    return (
      <div className="max-w-md mx-auto text-center py-16 px-4">
        ...
        <button
          type="button"
          onClick={() => login({ returnUrl: '/admin/campaigns' })}
  ...
  const posthogIssue = data
    ? !data.posthog.configured
      ? 'PostHog querying is not configured — showing link clicks only. This report runs on the pyre-landing-page Vercel project: set POSTHOG_PERSONAL_API_KEY and POSTHOG_PROJECT_ID there for the Production environment, then redeploy (env changes only reach new deployments).'
      : data.posthog.error
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-2">
        <div>
          ...
          <p className="text-xs text-white/40 mt-1">
            Short-link clicks from Redis; visits, signups, and bookings attributed by
            first-touch utm_campaign in PostHog.
          </p>
        </div>
                        ...
                        <td colSpan={7} className="px-6 py-3">
                          {campaign.shortlinks.length === 0 ? (
                            <p className="text-xs text-white/40">
                              No short links for this campaign ({campaign.linkCount} tracked
                              link{campaign.linkCount === 1 ? '' : 's'} without short codes).
                            </p>
                          ) : (
