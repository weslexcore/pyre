...
- **Sizes are an enum.** `square | portrait | landscape | reel | story`. See `scripts/lib/sizes.ts` for dimensions. Don't invent a new key — extend that file if a new size is genuinely needed.
