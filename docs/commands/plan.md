...
8.  **Identify Relevant Files:** Based on the tasks and PRD, identify potential files that will need to be created or modified. List these under the `Relevant Files` section, including corresponding test files if applicable. Include whether the file is new - to be created by this feature - or existing.
9.  **Generate Final Output:** Combine the parent tasks, sub-tasks, relevant files, and notes into the final Markdown structure.
10.  **Save Task List:** Save the generated document in the `/docs/features/` directory with the filename `<N>-SHORT_DESCRIPTION_TASKS.md` file with the same number and description as the PRD file. (e.g., if the input was `0001-USER_PROFILE_EDITING_PRD.md`, the output is `0001-USER_PROFILE_EDITING_TASKS.md`). Use all UPPERCASE for the file names.
...
- `path/to/potential/file1.ts` (New or Existing)- Brief description of why this file is relevant (e.g., Contains the main component for this feature).
- `path/to/file1.test.ts` (New or Existing) - Unit tests for `file1.ts`.
- `path/to/another/file.tsx` (New or Existing) - Brief description (e.g., API route handler for data submission).
- `path/to/another/file.test.tsx` (New or Existing) - Unit tests for `another/file.tsx`.
- `lib/utils/helpers.ts` (New or Existing) - Brief description (e.g., Utility functions needed for calculations).
- `lib/utils/helpers.test.ts` (New or Existing) - Unit tests for `helpers.ts`.
...
- **Filename:** `<N>-SHORT_DESCRIPTION_TASKS.md` file with the same number and description as the PRD file. Use all UPPERCASE for the file name. 